    ...
    try {
      window.addEventListener('load', async (event) => {
        navigator.serviceWorker.register("/sw.js").then(
          (registration) => {console.log('Successfully registered service worker');},
...
async function getRecipes() {
  ...
  //            declared above
    RECIPE_URLS.forEach(async (elem) => {
  ...
  //            will be in the catch portion.
      try {
        const recipe = await fetch(elem);
        const rec_json = await recipe.json();
        rec.push(rec_json);
        if(RECIPE_URLS.lastIndexOf(elem) == (RECIPE_URLS.length - 1)){
          resolve(rec);
        }
